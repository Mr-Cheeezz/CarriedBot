...
const  CarriedId = 72121088 // roblox id for getting game and playtime
...
export const responses = {
    ...
    const game = await ROBLOX_FUNCTIONS.getPresence(CarriedId).then((r)=>{return r.lastLocation})
    ...
    if (onlineStatus > 30) {
        return client.say(
            CHANNEL_NAME, `@${target}, Carried is not playing anything right now.`);
    }
    if (game != 'Website') {
        return client.say(
            CHANNEL_NAME, `@${target}, Carried is currenyly playing ${game}`);
    }
    return client.say(
        CHANNEL_NAME, `@${target}, Carried is currently switching games.`);
    },
    servertype (client, target, message = null) {
        ...
        if (currentMode == "!link.on") {
          ...
          client.say(CHANNEL_NAME, `${BOT} C is currently 1v1ing viewers in his private server, type 1v1 in the chat to have a chance of being picked @${target}`);
        } else if (currentMode == "!join.on") {
          client.say(CHANNEL_NAME, `${BOT} This is a public server type !roblox to join  Carried @${target}`);
        }
    },
    ...
    "1v1" (client, target, message = null) {
        ...
        if (currentMode == "!1v1.on") {
          client.say(CHANNEL_NAME, `${BOT} Hey, @${target} type 1v1 in the chat ONCE to have a chance to 1v1  Carried.`);
        } else {
          client.say(CHANNEL_NAME, `${BOT}  Carried is not currently doing 1v1s @${target}`);
        }
    },
    user (client, target, message = null) {
        client.say(CHANNEL_NAME, `${BOT} @${target}  Carried's username is BeingCarried`);
    },
